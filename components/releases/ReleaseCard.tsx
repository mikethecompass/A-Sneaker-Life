...
export function ReleaseRow({
  ...
  return (
    <div className="flex items-center gap-4 sm:gap-5 py-4 border-b border-gray-100 last:border-0 group">
      <Link
        href={stockxUrl ?? affiliateUrl ?? "#"}
        target="_blank"
        rel="noopener noreferrer"
        className="shrink-0 relative w-[80px] h-[80px] sm:w-[100px] sm:h-[100px] bg-gray-50 border border-gray-100 rounded overflow-hidden"
      >
        {imageUrl ? (
      ...
      <div className="flex-1 min-w-0">
        ...
        <Link href={stockxUrl ?? affiliateUrl ?? "#"} target="_blank" rel="noopener noreferrer">
          <h2 className="text-base sm:text-lg font-bold leading-tight line-clamp-2 text-black group-hover:underline mb-0.5">
            {title}
          </h2>
        </Link>
      ...
      <div className="shrink-0 flex flex-col items-end gap-1 min-w-[100px]">
        {retailPrice ? (
          <span className="text-lg font-bold text-black">{formatPrice(retailPrice, currency)}</span>
        ) : null}
        <span className="text-sm font-semibold text-green-600">
          {formatReleaseDate(releaseDate)}
        </span>
        {resalePrice ? (
          <span className="text-xs text-gray-400">Resale ~{formatPrice(resalePrice, currency)}</span>
        ) : null}
        <div className="mt-1">
          {stockxUrl && (
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wide bg-green-500 hover:bg-green-600 text-white px-3 py-2 rounded-full transition-colors whitespace-nowrap"
            >
              StockX
            </a>
          )}
