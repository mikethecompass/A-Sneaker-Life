...
export function ReleaseRow({ _id, title, slug, brand, colorway, sku, imageUrl, retailPrice, resalePrice, currency = "USD", releaseDate, releaseTime, affiliateUrl, stockxUrl }: ReleaseItem) {
  ...
  return (
    <div className="flex items-center gap-4 sm:gap-5 py-4 border-b border-gray-100 last:border-0 group">
      ...
      <div className="shrink-0 flex flex-col items-end gap-1 min-w-[100px]">
        ...
        {stockxUrl && (
          <a href={stockxUrl} target="_blank" rel="noopener noreferrer" className="mt-1 flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wide bg-green-500 hover:bg-green-600 text-white px-3 py-2 rounded-full transition-colors whitespace-nowrap">
            StockX
          </a>
        )}
      </div>
