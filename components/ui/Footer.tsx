    ...
    <footer className="border-t border-gray-200 mt-16 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-10">
          <div>
            <p className="text-sm font-bold uppercase tracking-[0.2em] mb-3 text-gray-900">A Sneaker Life</p>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {[{ label: "All Deals", href: "/deals" }, { label: "Upcoming Releases", href: "/releases" }, { label: "Videos", href: "/videos" }].map((link) => (
                <li key={link.href}>
                  <Link href={link.href} className="text-xs text-gray-500 hover:text-gray-900 transition-colors">{link.label}</Link>
                </li>
              ))}
            </ul>
          </div>
          <div>
            ...
            <ul className="space-y-2">
              <li><a href="https://twitter.com/ASneakerLife" target="_blank" rel="noopener noreferrer" className="text-xs text-gray-500 hover:text-gray-900 transition-colors">X / Twitter</a></li>
              <li><a href="https://youtube.com/@ASneakerLife" target="_blank" rel="noopener noreferrer" className="text-xs text-gray-500 hover:text-gray-900 transition-colors">YouTube</a></li>
            </ul>
          </div>
        </div>
        <div className="border-t border-gray-200 pt-6">
          ...
          <p className="text-xs text-gray-500 mt-3">&copy; {year} A Sneaker Life. All rights reserved.</p>
        </div>
      </div>
    </footer>
  );
